...
function verifyBuild() {
  ...
  const missingFiles = essentialFiles.filter(file => !fs.existsSync(path.resolve(file)));

  if (missingFiles.length === 0) {
    console.log('All essential files are present in the build output.');
  } else {
    logger.error('The following essential files are missing from the build output:');
